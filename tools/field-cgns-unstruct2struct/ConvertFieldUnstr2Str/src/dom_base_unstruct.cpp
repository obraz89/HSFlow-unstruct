...
#include "common_procs_unstruct.h"

t_DomBase* g_pDomUnst;
...
void t_Zone::makeCellConnectivity() {
	...
	// NB: ghosts disabled
	hsLogMessage("NB: cell connectivity: ghosts disabled");
...
//
	...
	hsLogMessage("Worker #%d own range of zones: [%d, %d]", G_State.mpiRank, iZneMPIs, iZneMPIe);
...
void t_DomBase::initializeFromCtxStage1() {
	...
	for (int i = 0; i < nZones; i++) {
		...
		const t_CGNSZone& cgZne = G_CTXUnst.cgZones[i];
...
void t_DomBase::loadCells() {
	...
	const t_CGNSContext& ctx = G_CTXUnst;
