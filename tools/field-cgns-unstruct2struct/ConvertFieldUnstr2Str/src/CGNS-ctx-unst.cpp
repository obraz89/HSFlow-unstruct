		...
		for (int iZone = 0; iZone < this->nZones; iZone++) {
			...
			for (index_sect = 1; index_sect <= nsections; index_sect++)
			{
				...
				if (itype == CG_QUAD_4 || itype == CG_TRI_3) {
					...
					bool is_bc_section = false;
					std::string SectionBCFamName;

					cgZne.getBCList().detectSectionFamilyName(
						istart, iend, is_bc_section, SectionBCFamName);
					...
					if (is_bc_section) {
						...
						pPatch = new t_CGSection(sectionname, istart, iend);
...
bool t_CGNSContext::parseBCs(int cgZneID) {

	t_CGZoneBCList& bc_list = cgZones[cgZneID-1].getBCList();
	...
	for (int iBC = 1; iBC <= nBCs; ++iBC) {
		...
		hsLogMessage("BC with Family Name %s has idxrng_start=%d and idxrng_end=%d", 
			szPatchName, idxRng[0], idxRng[1]);
		...
		{
			hsLogWarning("t_CGNSContext::parseBCs: failed to read bc set as family name");
			strcpy_s(szBC, szPatchName);
		}

		bc_list.push_back(t_CGBC(szBC, idxRng[0], idxRng[1]));
	}
